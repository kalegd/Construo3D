...
export const createInputOfType = (dataType, initialValue, name, instance) => {
    if(dataType == "color") {
...
function _createFloatInput(initialValue, name, instance) {
    ...
    $(input).on("blur", function () {
        let validValue = isNaN(parseFloat(div.getValue()))
            ? div.lastValidValue
...
function _createIntegerInput(initialValue, name, instance) {
    ...
    div.getName = function() { return name; };
    $(input).on("blur", function () {
        let validValue = isNaN(parseInt(div.getValue()))
            ? div.lastValidValue
...
function _createAssetInput(initialValue, name, instance) {
    ...
    if(!initialValue) {
        ...
        bText = document.createTextNode(global.dataStore.getAssetName(initialValue) + "\xa0\xa0");
    }
    b.append(bText);
    span.append(text);
    span.append(b);
...
function _createAudioInput(initialValue, name, instance) {
    ...
    if(!initialValue) {
        ...
        bText = document.createTextNode(global.dataStore.getAudioName(initialValue) + "\xa0\xa0");
    }
    b.append(bText);
    span.append(text);
    span.append(b);
...
function _createAudioDialog(instance, name, div, input, b) {
    ...
    dialogDiv.append(h1);
    dialog.append(dialogDiv);
    for(let audioId in global.dataStore.audios) {
        let audio = global.dataStore.audios[audioId];
        let a = document.createElement("a");
...
function _createImageInput(initialValue, name, instance) {
    ...
    if(!initialValue) {
        ...
        bText = document.createTextNode(global.dataStore.getImageName(initialValue) + "\xa0\xa0");
    }
    b.append(bText);
    span.append(text);
    span.append(b);
...
function _createImageDialog(instance, name, div, input, b) {
    ...
    dialogDiv.append(h1);
    dialog.append(dialogDiv);
    for(let imageId in global.dataStore.images) {
        let image = global.dataStore.images[imageId];
        let a = document.createElement("a");
        let img = new Image();
...
export const replaceAllInObject = (object, oldValue, newValue) => {
...
export const getTerrainIntersection = (object, terrain) => {
...
export const getLocalTerrainIntersection = (object, terrain) => {
    let origin = object.position.clone();
...
export const vectorProjectionOnPlane = (direction, planeNormal) => {
